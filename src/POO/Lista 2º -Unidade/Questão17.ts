...
while (comando.toLowerCase() !== "encerrar") {
    comando = String(prompt("Digite 'cadastrar' para novo usuário, 'passar' para registrar acesso ou 'encerrar' para finalizar:"))

    if (comando.toLowerCase() === "cadastrar") {
        let tipo = String(prompt("Tipo de usuário: 1-Aluno | 2-Servidor")).trim()
    ...
    else if (comando.toLowerCase() === "passar") {
...
for (let i = 0; i < historico.length; i++) {
    mensagens += historico[i].identificar() + "\n"
